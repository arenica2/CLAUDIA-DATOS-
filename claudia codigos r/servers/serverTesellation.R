...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$userLogin,{
    ...
    if(loginSuccess) {
      ...
      #                             message = "Identificatio??n incognido")
      # })
    }
  ...
  observeEvent(input$inputSubmit, {
    ...
    if (input$P == '' ||
      ...
      observe({
        session$sendCustomMessage(type = 'validation-message',
                                  message = "Ingrese la observaci??n sobre el codigo en la caja de texto")
      })
      ...
      observe({
        session$sendCustomMessage(type = 'validation-message',
                                  message = "No puede marcar en chiris o rastros sin haber seleccionado el lugar de inspecci??n adecuado ")
      })
      ...
      if(groupParameters$WRITE_ACCESS) {
        ...
        observe({
          session$sendCustomMessage(type = 'validation-message',
                                    message = "Los datos fueron GUARDADOS con ??xito. Gracias.")
        })
        cleanData()
      } else {
        observe({
          session$sendCustomMessage(type = 'validation-message',
                                    message = "ALERTA!! Los datos no pudieron ser guardados con ??xito, por favor revise")
        })
      }
  ...
  cleanData <- function(){
    #Almacena el tiempo en que se empieza a ingresar la informaci??n
    date_start <<- Sys.time()
  ...
  getLocalityData <- function(){
    ...
    lastInspections[lastInspections$STATUS_INSPECCION == "inspeccion",4] <- "inspecci??n"
  ...
  observeEvent(input$load, {
    source(file.path("~/PETM-shiny/triangulation/delaunayTriangulationFunctions.R"))
    ...
    if (!is.null(input$locality)) {
      ...
      listTessellation <- getTessellation(copy, dataInspecciones)
      polyDF <- listTessellation[[1]]
      polyVertices <- listTessellation[[2]]
    ...
    for (z in 1:length(polyVertices)) {
      leafletMap %>% addPolygons(
        ...
        label = paste0(as.character(polyDF$nHouseUninspect[z])," casas no inspeccionadas"),
...
#    )
  ...
  #Funcion que me devolver?? las viviendas a las que se tiene que volver el dia en que se consulta
  ...
  #Funcion que me devolver?? las viviendas que se ingresaron en el dia
